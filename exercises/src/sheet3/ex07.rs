...
/// check if the Line::new function works as expected
fn test_new_line(){
    let line = line::Line::new(Point::new(0.0, 0.0), Point::new(1.0, 1.0));
}

#[runtest()]
#[overwrite(impl Point::new in point)]
#[overwrite(impl Line::new in line)]
/// check if Line::Contain works as expected
fn test_line(){
    let line = line::Line::new(Point::new(0.0, 0.0), Point::new(1.0, 1.0));
    assert!(line.contains(&Point::new(0.5, 0.5)).is_ok());
    assert!(line.contains(&Point::new(0.5, 0.6)).is_err());
    assert!(line.contains(&Point::new(1.5, 1.5)).is_ok());
}
...
#[overwrite(impl Point::new in point)]
/// tests if the distance metric is correctly defined.
fn test_distance_on_random_points(){
...
#[runtest(1.0)]
fn test_line() {
    use {line::Line, point::Point};
    let a = Point::new(1., 1.);
    let b = Point::new(3., 3.);

    let line = Line::new(a, b);

    assert_eq!(line.contains(&Point::new(2., 2.)), Ok(()));
    assert_eq!(line.contains(&Point::new(1., 1.)), Ok(()));
    assert_eq!(line.contains(&Point::new(3., 3.)), Ok(()));

    assert_eq!(line.contains(&Point::new(0., 5.)), Err("Not contained"));
}
*/